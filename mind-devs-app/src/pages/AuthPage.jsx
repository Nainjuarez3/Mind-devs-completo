...
import { API_URL } from '../config'; // <--- 1. IMPORTANTE: IMPORTAR LA CONFIGURACIÓN
...
const AuthPage = ({ navigateTo, initialView = 'login' }) => {
  ...
  const handleLogin = async (e) => {
    e.preventDefault();
    try {
      // 2. USAR API_URL EN LUGAR DE LOCALHOST
  ...
  const solicitarRecuperacion = async (e) => {
    ...
    try {
        ...
        if (res.ok) { alert("Código enviado."); setRecoveryStep(2); setCode(['', '', '', '']); } 
  ...
  const confirmarRecuperacion = async (e) => {
    ...
    try {
        ...
        if (res.ok) { alert("¡Contraseña actualizada!"); setView('login'); setRecoveryStep(1); setPassword(''); } 
        else { const data = await res.json(); alert("Error: " + data.error); }
    } catch (error) { console.error(error); }
  };
