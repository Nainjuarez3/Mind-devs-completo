...
const LevelMapPage = ({ course, navigateTo, onLevelSelect }) => {
    // ... (Tus estados y useEffects existentes NO cambian) ...
    // ... (Solo copiaremos la parte visual del return) ...
    ...
    useEffect(() => {
        const fetchProgreso = async () => {
            try {
                const res = await fetch(`http://localhost:3000/progreso/${usuario.id}/${course}`);
    ...
    const handleNodeClick = async (levelNumber) => {
        ...
        try {
            const res = await fetch(`http://localhost:3000/lecciones/${course}/${levelNumber}`);
            if (res.ok) {
                const data = await res.json();
    ...
    return (
        <div className="min-h-screen bg-white dark:bg-gray-900 flex flex-col font-sans relative transition-colors">
            ...
            {/* 2. AQUÍ PONEMOS LA NAVBAR GLOBAL */}
            <Navbar navigateTo={navigateTo} />

            {/* 3. SUB-HEADER DEL CURSO (Barra de color) */}
            ...
            {/* --- MODAL (Igual que antes, sin cambios en lógica) --- */}
            {isModalOpen && (
                        ...
                        {modalLoading ? (<div className="p-10 flex justify-center"><Loader className="animate-spin text-mind-primary" /></div>) : (
                            <>
                                <div className={`p-6 ${themeColor} text-white text-center`}>
                                    ...
                                    <h2 className="text-2xl font-bold uppercase tracking-wide">{selectedLevelInfo?.titulo || "Nivel " + selectedLevelInfo?.nivel}</h2>
